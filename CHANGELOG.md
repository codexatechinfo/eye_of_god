...
- Timeline (aba Trilho) podia travar silenciosamente numa abertura "fria" do app (antes de qualquer
  ...
  existia. Ver Adendo 1 da [ADR 0003](docs/adr/0003-rbac-multi-tenant.md) e Adendo 21 da [ADR
  0038](docs/adr/0038-sistema-de-design-e-restyle-trilho.md).
...
- Scraper de Acompanhamento ganha um "modo profundo": a primeira extração bem-sucedida de cada dia
  volta a abrir a OS de cada livro (só essa vez) pra aprender de verdade quais UCs foram
  enviadas a leitura — fecha a lacuna de `coordenadas_ucs_mineradas` poder estar desatualizada pra
  um livro reatribuído no dia. Gravado numa tabela nova dedicada (`roster_ucs_extracao_diaria`),
  sem tocar em nenhuma tela/consulta existente. Todo ciclo seguinte do mesmo dia continua no modo
  rápido de sempre (~30s-3min, sem abrir OS). Abrir cada OS via navegador (testado com 1 e depois 5
  abas em paralelo) ficou lento e instável demais na prática (10-12h pro catálogo do dia) — trocado
  por POST HTTP direto (sem navegador nenhum, réplica exata da requisição que o site dispara),
  testado ao vivo: **100% de cobertura em ~18-32min** (1091/1091 e depois 1556/1556 livros,
  confirmado direto no banco). Ver [ADR
  0039](docs/adr/0039-modo-profundo-diario-acompanhamento-roster-real-de-ucs.md).
...
- Cabeçalho da timeline do colaborador ganhou regional, cargo e % de bateria ao lado do nome — só
  ...
  vazado pros dois). Ver Adendo 4/5 da [ADR 0036](docs/adr/0036-aba-monitoramento-colaborador.md).
...
- Popover de filtro por coluna: fechar no scroll (fix acima) fechava o próprio popover que tinha
  acabado de abrir — focar o campo de busca já causa um scroll implícito em alguns navegadores, e
  rolar a tabela de propósito também fechava o filtro em uso. Scroll/resize agora reposicionam o
